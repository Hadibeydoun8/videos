...
def rev_to_rgba(alpha):
    ...
    hue_list = [
        0, 0.5 / 6.0, 1 / 6.0, 1.1 / 6.0, 2 / 6.0, 3 / 6.0, 4 / 6.0, 5 / 6.0
    ]
...
class EquationSolver1d(GraphScene, ZoomedScene):
    CONFIG = {
        ...
        "show_y_as_deviation":
        False,  # Displays y-values as deviations from target,
    }
    ...
    def drawGraph(self):
        ...
        if self.graph_label is not None:
            curve_label = self.get_graph_label(self.graph,
                                               self.graph_label,
                                               x_val=4,
                                               direction=LEFT)
        ...
        if self.show_target_line:
            ...
            target_line_label.next_to(target_line_object.get_left(),
                                      UP + RIGHT)
    ...
    def solveEquation(self):
        ...
        lower_color = color_by_comparison(self.func(self.initial_lower_x),
                                          self.targetY)
        upper_color = color_by_comparison(self.func(self.initial_upper_x),
                                          self.targetY)
        ...
        leftBrace.move_to(self.coords_to_point(
            lowerX, self.base_line_y))  # , aligned_edge = RIGHT)
        ...
        rightBrace.move_to(self.coords_to_point(
            upperX, self.base_line_y))  # , aligned_edge = LEFT)
        ...
        downBrace.move_to(self.coords_to_point(0,
                                               lowerY))  # , aligned_edge = UP)
        ...
        upBrace.move_to(self.coords_to_point(0,
                                             upperY))  # , aligned_edge = DOWN)
        ...
        x_guess_line = Line(lowerDotXPoint,
                            upperDotXPoint,
                            color=WHITE,
                            stroke_width=10)

        lowerGroup = Group(lowerDot, leftBrace, downBrace, lowerXLine,
                           lowerYLine, x_guess_line)

        upperGroup = Group(upperDot, rightBrace, upBrace, upperXLine,
                           upperYLine, x_guess_line)
        ...
        self.play(FadeIn(initialLowerXDot), FadeIn(leftBrace),
                  FadeIn(leftBraceLabel))
        ...
        self.play(FadeIn(initialLowerYDot), FadeIn(downBrace),
                  FadeIn(downBraceLabel))
        self.add_foreground_mobjects(initialLowerYDot, downBrace)
        self.add(downBraceLabelAnimation)

        self.wait()

        self.play(FadeIn(initialUpperXDot), FadeIn(rightBrace),
                  FadeIn(rightBraceLabel))
        ...
        self.play(FadeIn(initialUpperYDot), FadeIn(upBrace),
                  FadeIn(upBraceLabel))
        ...
        for i in range(self.num_iterations):
            if i == self.iteration_at_which_to_start_zoom:
                ...
                self.play(lowerDot.scale, inverseZoomFactor, upperDot.scale,
                          inverseZoomFactor)
            ...
            def makeUpdater(xAtStart, fixed_guess_x):
                ...
                def updater(group, alpha):
                    ...
                    guess_line.put_start_and_end_on(xAxisPoint,
                                                    fixed_guess_point)
                    return group
            ...
            midXBrace.move_to(
                self.coords_to_point(midX, self.base_line_y) + OUT)

            # We only actually add this much later
            midXPoint = Dot(self.coords_to_point(midX, self.base_line_y) + OUT,
                            color=sign_color)

            x_guess_label_caption = TexText("New guess: x = ",
                                            fill_color=midColor)
            ...
            midXLine = DashedLine(self.coords_to_point(midX, self.base_line_y),
                                  midCoords,
                                  color=midColor)
            self.play(ShowCreation(midXLine))
            midDot = Dot(midCoords, color=sign_color)
            if (self.iteration_at_which_to_start_zoom is not None
                    and i >= self.iteration_at_which_to_start_zoom):
                midDot.scale(inverseZoomFactor)
            self.add(midDot)
            midYLine = DashedLine(midCoords,
...
def make_alpha_winder(func, start, end, num_checkpoints, cheap=False):
    ...
    for i in range(num_checkpoints - 1):
        check_points[i + 1] = resit_near(func(start + (i + 1) * step_size),
                                         check_points[i])
    ...
    def return_func(alpha):
        ...
        if cheap:
            return check_points[
                index]  # A more principled test that at least returns a reasonable answer
        else:
...
class WalkerAnimation(Animation):
    ...
    def __init__(self,
        ...
        base_walker = (Dot().scale(5 * 0.35).set_stroke(
            walker_stroke_color, 2))  # PiCreature().scale(0.8 * 0.35)
        ...
        if self.show_arrows:
            ...
            if self.scale_arrows:
                ...
                self.mobject.arrow.scale(
                    size *
                    0.3,  # Hack constant; we barely use this feature right now
                    about_point=self.mobject.arrow.get_start(),
                )


def walker_animation_with_display(walk_func,
                                  val_func,
                                  coords_to_point,
    ...
    walker_anim = WalkerAnimation(walk_func=walk_func,
                                  val_func=val_func,
    ...
    if number_update_func is not None:
        ...
        display_anim = ChangingDecimal(
            ...
            **kwargs)
        anim_group = AnimationGroup(walker_anim,
                                    display_anim,
                                    rate_func=linear)
        return anim_group
    else:
...
def LinearWalker(start_coords,
    ...
    def walk_func(alpha):
        return interpolate(start_coords, end_coords, alpha)
...
class ColorMappedByFuncScene(Scene):
    CONFIG = {
        ...
        "show_output": False,
        "hide_background":
        False,  # Background used for color mapped objects, not as background
    }
    ...
    def setup(self):
        ...
        def mini_hasher(p):
            ...
            if rgba[3] != 1.0:
                print("Warning! point_to_rgba assigns fractional alpha",
                      rgba[3])
            return tuple(rgba)
        ...
        if self.in_background_pass:
            ...
            print(
                "The background file already exists; this will only be a video pass"
            )

    def construct(self):
        if self.in_background_pass:
            self.camera.set_background_from_func(lambda x_y: point_to_rgba(
...
class PiWalker(ColorMappedByFuncScene):
    ...
    def construct(self):
        ...
        polygon_anim = ShowCreation(polygon,
        ...
        for i in range(len(walk_coords)):
            ...
            val_alpha_func = (
                lambda a, start_coords=start_coords, end_coords=end_coords:
                self.func(interpolate(start_coords, end_coords, a)))

            if self.display_wind:

                def clockwise_val_func(p):
                    return -point_to_rev(self.func(p))

                alpha_winder = make_alpha_winder(clockwise_val_func,
                                                 start_coords, end_coords,
                                                 self.num_checkpoints)
                number_update_func = (
                    lambda alpha, alpha_winder=alpha_winder, start_wind=start_wind: alpha_winder(alpha) - alpha_winder(
                        0) + start_wind)
                start_wind = (0 if i + 1 in self.wind_reset_indices else
                              number_update_func(1))
            elif self.display_size:
                # We need to do this roundabout default argument thing to get the closure we want,
                # so the next iteration changing val_alpha_func doesn't change this closure
                number_update_func = (lambda a, val_alpha_func=val_alpha_func:
                                      point_to_rescaled_size(val_alpha_func(a))
            ...
            new_anim = LinearWalker(
                ...
                if self.color_foreground_not_background else BLACK,
            ...
            if self.display_odometer:
                ...
                # We need to do this roundabout default argument thing to get the closure we want,
                # so the next iteration changing val_alpha_func doesn't change this closure
                def rev_func(a, val_alpha_func=val_alpha_func):
                    return point_to_rev(val_alpha_func(a))
...
class PiWalkerCircle(PiWalker):
    ...
    def setup(self):
        ...
        N = self.num_steps
        self.walk_coords = [
            r * np.array((np.cos(i * TAU / N), np.sin(i * TAU / N)))
            for i in range(N)
        ]
        PiWalker.setup(self)
...
class RectangleData:
    ...
    def split_line_on_dim(self, dim):
        ...
        if dim == 0:
            ...
            print(
                "RectangleData.split_line_on_dim passed illegitimate dimension!"
            )
...
class EquationSolver2dNode(object):
    ...
    def display_in_series(self):
        return Succession(self.first_anim,
                          *[n.display_in_series() for n in self.children])
...
class EquationSolver2d(ColorMappedObjectsScene):
    CONFIG = {
        ...
        "use_cheap_winding_numbers":
        False,  # To use this, make num_checkpoints large
    }
    ...
    def construct(self):
        ...
        def Animate2dSolver(
            ...
            def draw_line_return_wind(start,
                                      end,
                                      start_wind,
                                      should_linger=False,
                                      draw_line=True):
                ...
                def rebased_winder(alpha):
                    return alpha_winder(alpha) - a0 + start_wind
                ...
                walker_anim = LinearWalker(
                    ...
                    coords_to_point=num_plane.coords_to_point,
                    val_func=self.
                    func,  # Note: This is the image func, and not logic_func
                    number_update_func=rebased_winder
                    if self.show_winding_numbers else None,
                    remover=True,
                    walker_stroke_color=WALKER_LIGHT_COLOR,
                ...
                opt_line_anim = (ShowCreation(colored_line)
                                 if draw_line else empty_animation)

                line_draw_anim = AnimationGroup(opt_line_anim,
                                                walker_anim,
                                                run_time=run_time,
                                                rate_func=rate_func)
            ...
            if self.show_cursor:
                ...
                cursor.move_to(
                    num_plane.coords_to_point(center_x, center_y) + 10 * IN)
            ...
            if total_wind == 0:
                ...
                fill_rect = polygonObject = Polygon(*points,
                                                    fill_opacity=0.8,
                                                    color=GREY_D)
                return EquationSolver2dNode(Succession(anim,
                                                       FadeIn(fill_rect)))
            else:
                ...
                children = [
                    Animate2dSolver(
                        ...
                        manual_wind_override=child(manual_wind_override,
                                                   index),
                    ) for (index,
                           (sub_rect,
                            side_to_draw)) in enumerate(sub_rect_and_sides)
                ]
        ...
        def rect_rate(alpha):
            ...
            if time_in < 3 * run_time_base:
                ...
                alpha_in_last_leg = fdiv(time_in_last_leg,
                                         run_time_with_lingering)
                return interpolate(0.75, 1, linger_rate(alpha_in_last_leg))

        border_anim = ShowCreation(border,
...
class TestRotater(Scene):
    ...
    def construct(self):
        ...
        self.play(
            FuncRotater(test_line, rev_func=lambda x: x % 0.25, run_time=10))
...
class OdometerScene(ColorMappedObjectsScene):
    CONFIG = {
        ...
        "rotate_func":
        lambda x: 2 * np.sin(2 * x * TAU),  # This is given in terms of CW revs
        "run_time": 40,
    ...
    def construct(self):
        ...
        if self.dashed_line_angle:
            ...
            # Clockwise rotation
            dashed_line.rotate(-self.dashed_line_angle * TAU,
                               about_point=ORIGIN)
        ...
        def display_func(alpha):
            return self.rotate_func(alpha) + display_val_bias
...
class SecondSqrtScene(FirstSqrtScene):
    CONFIG = {
        "graph_label":
        FirstSqrtSceneConfig["graph_label"] + " - " + str(shiftVal),
...
class RewriteEquation(Scene):
    ...
    def construct(self):
        ...
        self.add(f_old, equals_old, equals_old_2,
                 g_old)  # , where_old, qmark_old)
        self.wait()
        self.play(
...
class VectorField(Scene):
    ...
    def construct(self):
        ...
        x_min, y_min = num_plane.point_to_coords(FRAME_X_RADIUS * LEFT +
                                                 FRAME_Y_RADIUS * UP)
        x_max, y_max = num_plane.point_to_coords(FRAME_X_RADIUS * RIGHT +
                                                 FRAME_Y_RADIUS * DOWN)
...
class HasItsLimitations(Scene):
    ...
    def construct(self):
        ...
        new_curved_arrow.shift((new_input_dot.get_center() -
                                new_curved_arrow.get_points()[0]) - input_diff)

        new_output_dot = output_dot.copy().move_to(
            new_curved_arrow.get_points()[-1] + output_diff)
        new_output_label = output_label.copy().next_to(new_output_dot,
                                                       UP + RIGHT)

        dot_objects = Group(input_dot, input_label, output_dot, output_label,
                            curved_arrow)
...
class NumberLineScene(Scene):
    ...
    def construct(self):
        ...
        border_2d = Polygon(*random_points,
                            stroke_color=border_color,
                            stroke_width=stroke_width)

        filling_2d = Polygon(*random_points,
                             fill_color=inner_color,
                             fill_opacity=0.8,
                             stroke_width=stroke_width)
...
class Initial2dFuncSceneWithoutMorphing(Initial2dFuncSceneBase):
    ...
    def setup(self):
        ...
        right_camera = MappingCamera(mapping_func=self.func,
                                     **self.camera_config)
        split_screen_camera = SplitScreenCamera(left_camera, right_camera,
                                                **self.camera_config)
...
class DemonstrateColorMapping(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        self.play(
            ApplyMethod(circle.scale, scale_up_factor * scale_down_factor))
...
class LoopSplitScene(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        bottom_line = Line(br,
                           bl)  # Invisible; only used for surrounding circle
        ...
        def bullet_fade(start, end, mob):
            return UpdateFromAlphaFunc(
                mob,
                lambda m, a: m.set_fill(opacity=interpolate(start, end, a)))
        ...
        def line_fade(start, end, mob):
            return UpdateFromAlphaFunc(
                mob,
                lambda m, a: m.set_stroke(width=interpolate(start, end, a) *
                                          stroke_width),
            )

        def play_combined_fade(start, end, lines_vmobject, bullets):
            self.play(line_fade(start, end, lines_vmobject),
                      *bullet_list_fade(start, end, bullets))

        def play_fade_left(start, end):
            play_combined_fade(start, end, left_square_lines_vmobject,
                               left_square_bullets)

        def play_fade_right(start, end):
            play_combined_fade(start, end, right_square_lines_vmobject,
                               right_square_bullets)

        def play_fade_mid(start, end):
            play_combined_fade(start, end, midline_lines_vmobject,
                               midline_bullets)
        ...
        self.play(*bullet_list_fade(0, 1, left_square_bullets +
                                    right_square_bullets))
        self.wait()
        ...
        self.wait()
        self.play(FadeOut(inner_circle), line_fade(1, 0,
                                                   midline_lines_vmobject),
                  *bullet_list_fade(1, 0, midline_bullets))
        self.wait()

        # Repeat for effect, goes well with narration
        self.play(FadeIn(inner_circle), line_fade(0, 1,
                                                  midline_lines_vmobject),
...
class X5ThingWithRightHalfGreyed(SolveX5MinusXMinus1):
    CONFIG = {
        ...
        (1, None, (1, (0, None, None), (0, None, None))),
    }
...
class SolveX5MinusXMinus1_5Iterations(EquationSolver2d):
    CONFIG = {
        "func":
        plane_func_from_complex_func(lambda c: c**5 - c - 1),
        "num_iterations":
        5,
        "show_cursor":
        True,
        ...
        True,
        "manual_wind_override":
        (None, None, (None, (0, None, None), (0, None, None))),
    }
...
class Diagnostic(SolveX5MinusXMinus1_3Iterations):
    CONFIG = {
        ...
        "use_separate_plays":
        not False,  # This one isn't important to set any particular way, so let's leave it like this
        "use_fancy_lines": True,
...
class ParallelClip(EquationSolver2d):
    CONFIG = {
        "func": plane_func_by_wind_spec((-3, -1.3, 2), (0.1, 0.2, 1),
                                        (2.8, -2, 1)),
        "num_iterations": 5,
        "display_in_parallel": True,
    }
...
def diff_func(x):
    return np.cos(1.4 * (x - 0.1) * (np.log(x + 0.1) - 0.3) * TAU) / 2.1
...
tiny_loop_func = scale_func(plane_func_by_wind_spec((-1, -2), (1, 1), (1, 1)),
                            0.3)
...
class TinyLoopInOutputPlaneAroundNonZero(TinyLoopInInputPlaneAroundNonZero):
    CONFIG = {
        ...
        "camera_config": {
            "mapping_func": point3d_func_from_plane_func(tiny_loop_func)
        },
        "show_output": True,
        "show_num_plane": False,
    }
...
def big_loop_no_zeros_func(x_y5):
    return complex_to_pair(np.exp(complex(10, x_y5[1] * np.pi)))
...
class NoticeHowOnThisLoop(PiWalkerRect):
    CONFIG = {
        ...
        "start_y": -0.5,
        "walk_width":
        -1,  # We trace from bottom-right clockwise on this one, to start at a red point
        "walk_height": -1,
    }
...
class UhOhSalientStill(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        midline = Line(new_up + 0.5 * new_left,
                       0.5 * new_left,
                       stroke_width=border_stroke_width)
...
class ThreeTurnWalker(PiWalkerRect):
    CONFIG = {
        "func":
        plane_func_from_complex_func(lambda c: c**3 * complex(1, 1)**3),
        "double_up": True,

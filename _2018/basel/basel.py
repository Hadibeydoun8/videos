...
A = np.array([5.0, -3.0, 0.0])
B = np.array([-5.0, 3.0, 0.0])
...
class LightIndicator(VMobject):
    ...
    def measured_intensity(self):
        ...
        intensity = (self.light_source.opacity_function(distance) /
...
class UpdateLightIndicator(AnimationGroup):
    ...
    def __init__(self, indicator, intensity, **kwargs):
        ...
        target_foreground = indicator.copy().set_intensity(
        ...
        AnimationGroup.__init__(self, changing_decimal, change_opacity,
                                **kwargs)
    ...
    def __init__(self, light_sources_mob, factor, about_point=None, **kwargs):
        ...
        for submob in ls_target:
            ...
            if type(submob) == LightSource:
                ...
                submob.ambient_light.scale_about_point(factor,
                                                       new_sp.get_center())
...
class IntroScene(PiCreatureScene):
    ...
    def build_up_euler_sum(self):
        ...
        terms = [1.0 / n**2 for n in range(1, 6)]
    ...
    def build_up_sum_on_number_line(self):
        ...
        self.play(FadeIn(self.number_line), FadeIn(self.number_line_labels))
        self.wait()
        ...
        terms = [0] + [1.0 / (n**2) for n in range(1, max_n2 + 1)]
        ...
        self.arrow = CurvedArrow(P, Q, angle=TAU / 12, color=YELLOW)
...
class FirstLighthouseScene(PiCreatureScene):
    ...
    def show_lighthouses_on_number_line(self):
        ...
        for (i, term) in zip(list(range(len(euler_sum_above))),
                             euler_sum_above):
        ...
        intensities = np.cumsum(
            np.array([1.0 / n**2 for n in range(1, NUM_CONES + 1)]))
        ...
        for (i, light_source) in zip(list(range(NUM_VISIBLE_CONES)),
                                     light_sources[:NUM_VISIBLE_CONES]):
            indicator_start_time = (1.0 * (i + 1) * SWITCH_ON_RUN_TIME /
                                    light_source.radius *
                                    self.number_line.unit_size)
            ...
            self.play(
                ...
                FadeIn(
                    euler_sum_above[2 * i - 1],
                    run_time=SWITCH_ON_RUN_TIME,
                    rate_func=indicator_rate_func,
                ),
                # this last line *technically* fades in the last term, but it is off-screen
                ChangeDecimalToValue(
                    light_indicator.reading,
                    intensities[i],
                    rate_func=indicator_rate_func,
                    run_time=SWITCH_ON_RUN_TIME,
                ),
                ApplyMethod(
                    light_indicator.foreground.set_fill,
                    None,
                    opacities[i],
                    rate_func=indicator_rate_func,
                    run_time=SWITCH_ON_RUN_TIME,
        ...
        for (i, light_source) in zip(
                list(range(NUM_VISIBLE_CONES, NUM_CONES)),
                light_sources[NUM_VISIBLE_CONES:NUM_CONES],
        ):
            indicator_start_time = (0.5 * (i + 1) * FAST_SWITCH_ON_RUN_TIME /
                                    light_source.radius *
                                    self.number_line.unit_size)
...
class SingleLighthouseScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.setup_elements()
        self.setup_angle(
        )  # spotlight and angle msmt change when screen rotates
        self.rotate_screen()
    ...
    def setup_elements(self):
        ...
        self.play(SwitchOn(self.light_source.ambient_light))
        ...
        self.play(FadeIn(self.light_source.shadow), )
        ...
        self.play(FadeIn(self.light_source.spotlight))
    ...
    def setup_angle(self):
        ...
        self.play(ShowCreation(self.angle_arc),
...
class MorphIntoSunScene(PiCreatureScene):
    ...
    def morph_lighthouse_into_sun(self):
        ...
        self.play(Transform(self.light_source, self.sun))
...
class EarthScene(Scene):
    ...
    def construct(self):
        ...
        # screen
        self.screen = VMobject(stroke_color=WHITE,
                               stroke_width=SCREEN_THICKNESS)
        self.screen.set_points_as_corners([[0, -self.screen_height / 2, 0],
        ...
        self.play(FadeIn(earth), FadeIn(background_earth))
        ...
        polar_morty = morty.copy().scale(0.5).next_to(screen_on_earth,
                                                      DOWN,
                                                      buff=0.5)
        polar_morty.set_color(BLUE_C)

        self.play(Transform(self.screen, screen_on_earth),
...
class ScreenShapingScene(ThreeDScene):
    ...
    def setup_elements(self):
        ...
        # light source
        self.light_source = LightSource(opacity_function=inverse_quadratic(
            1, 5, 1),
            num_levels=NUM_LEVELS,
            radius=10,
            max_opacity=0.2
        ...
        # Morty
        self.morty = Mortimer().scale(0.3).next_to(self.screen,
                                                   RIGHT,
                                                   buff=0.5)
        ...
        self.play(SwitchOn(self.spotlight), self.light_source.dim_ambient)
    ...
    def create_brightness_rect(self):
        ...
        brightness_rect0 = Rectangle(width=0,
                                     height=self.screen_height).move_to(
                                         self.screen.get_center())
        self.add_foreground_mobject(brightness_rect0)

        self.brightness_rect = Rectangle(
            width=self.brightness_rect_height,
            height=self.brightness_rect_height,
            fill_color=YELLOW,
            fill_opacity=0.5,
        )
        ...
        self.play(ReplacementTransform(brightness_rect0, self.brightness_rect))
    ...
    def slant_screen(self):
        ...
        lower_slanted_screen_point = interpolate(
            lower_screen_point, self.spotlight.get_source_point(),
            SLANTING_AMOUNT)
        upper_slanted_screen_point = interpolate(
            upper_screen_point, self.spotlight.get_source_point(),
            -SLANTING_AMOUNT)
        ...
        self.slanted_screen = Line(
            lower_slanted_screen_point,
            upper_slanted_screen_point,
            path_arc=0,
            num_arc_anchors=10,
        )
    ...
    def left_shift_screen_while_showing_light_indicator(self):
        ...
        self.play(FadeOut(self.brightness_rect), FadeIn(self.indicator))
        ...
        self.left_shift = (self.screen.get_center()[0] -
                           self.spotlight.get_source_point()[0]) / 2
        ...
        self.play(
            ...
            self.morty.shift,
            [-self.left_shift, 0, 0],
            self.indicator.shift,
            [-self.left_shift, 0, 0],
            self.indicator.set_intensity,
    ...
    def right_shift_screen_while_showing_light_indicator_and_distance_arrow(
            self):
    ...
    def prove_inverse_square_law(self):
        ...
        fourfold_screen.scale(2,
                              about_point=self.light_source.get_source_point())
        ...
        self.play(Transform(self.unit_screen, fourfold_screen))
        ...
        self.play(ShowCreation(midline1), ShowCreation(midline2))
        ...
        ninefold_screen.scale(3,
                              about_point=self.light_source.get_source_point())

        self.play(Transform(self.unit_screen, ninefold_screen))
...
class BackToEulerSumScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        intensities = np.array([1.0 / (n + 1)**2 for n in range(NUM_CONES)])
        ...
        collection_point = np.array([-6.0, 2.0, 0.0])
        left_shift = 0.2 * LEFT
        ...
        for i in range(NUM_VISIBLE_CONES, NUM_CONES):
            indicator_start_time = (0.5 * (i + 1) * FAST_SWITCH_ON_RUN_TIME /
                                    light_source.ambient_light.radius *
                                    self.number_line.unit_size)
            indicator_stop_time = indicator_start_time + FAST_INDICATOR_UPDATE_TIME
            indicator_rate_func = squish_rate_func(  # smooth, 0.8, 0.9)
                smooth, indicator_start_time, indicator_stop_time)
        ...
        self.play(FadeOut(collected_indicators), FadeIn(sum_indicator))
...
class TwoLightSourcesScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        intensity = (ls1.ambient_light.opacity_function(distance1) /
                     indicator.opacity_for_unit_intensity)
        distance2 = get_norm(C - ls2.get_source_point())
        intensity += (ls2.ambient_light.opacity_function(distance2) /
                      indicator.opacity_for_unit_intensity)
        ...
        location = np.array([-3, -2.0, 0.0])
        self.play(ls3.move_source_to, location)
        location = np.array([6.0, 1.0, 0.0])
        self.play(ls3.move_source_to, location)
        location = np.array([5.0, 2.0, 0.0])
        self.play(ls3.move_source_to, location)
        ...
        # state the IPT
        theorem_location = np.array([3.0, 2.0, 0.0])
...
class IPTScene1(PiCreatureScene):
    ...
    def construct(self):
        ...
        if not show_detail:
            self.play(SwitchOn(ls1.ambient_light))
        ...
        self.play(SwitchOn(ls1.spotlight), SwitchOff(ls1.ambient_light))
        ...
        screen1p.move_to(C + (screen_width_b - screen_width_bp / 2) * RIGHT +
                         SCREEN_THICKNESS / 2 * DOWN)
        screen1p.rotate(-angle, about_point=C + screen_width_b * RIGHT)

        self.play(ls1.move_source_to, H, Transform(screen1, screen1p))

        # add and move the second light source and screen
        ls2 = ls1.deepcopy()
        ls2.move_source_to(A)
        screen2 = Rectangle(
            width=screen_width_a,
        ...
        if not show_detail:
            self.play(SwitchOn(ls2.ambient_light))

        self.wait()

        self.play(SwitchOn(ls2.spotlight), SwitchOff(ls2.ambient_light))
        ...
        screen2p.move_to(C + (screen_width_a - screen_width_ap / 2) * UP +
                         screen_thickness_a / 2 * LEFT)
...
class IPTScene2(Scene):
    ...
    def construct(self):
        ...
        indy1 = LightIndicator(color=LIGHT_COLOR,
                               show_reading=False,
                               radius=indy_radius)
        ...
        indy2 = LightIndicator(color=LIGHT_COLOR,
                               show_reading=False,
                               radius=indy_radius)
        ...
        indy3 = LightIndicator(color=LIGHT_COLOR,
                               show_reading=False,
                               radius=indy_radius)
        ...
        formula = VGroup(indy1, plus_sign, indy2, equals_sign, indy3)
...
class InscribedAngleScene(ThreeDScene):
    ...
    def construct(self):
        ...
        def shift_scene(v):
            self.play(self.zoomable_mobs.shift, v, self.unzoomable_mobs.shift,
                      v)
        ...
        baseline = VMobject()
        baseline.set_points_as_corners([[-8, BASELINE_YPOS, 0],
                                        [8, BASELINE_YPOS, 0]])
        baseline.set_stroke(
            width=0)  # in case it gets accidentally added to the scene
        self.zoomable_mobs.add(baseline)  # prob not necessary

        self.obs_dot = Dot(OBSERVER_POINT, fill_color=DOT_COLOR)
        self.ls0_dot = Dot(OBSERVER_POINT + 2 * LAKE0_RADIUS * UP,
                           fill_color=WHITE)
        ...
        # first lighthouse
        original_op_func = inverse_quadratic(LIGHT_MAX_INT, AMBIENT_SCALE,
                                             LIGHT_CUTOFF)
        ...
        self.play(FadeIn(indicator_reading))
        ...
        def indicator_wiggle():
            ...
            self.play(
                ScaleInPlace(indicator,
                             INDICATOR_WIGGLE_FACTOR,
                             rate_func=wiggle),
        ...
        def position_for_index(i, step, scaled_down=False):
            ...
            if scaled_down:
                return position.scale_about_point(self.obs_dot.get_center(),
                                                  0.5)
            else:
                return position
        ...
        def construction_step(n,
            ...
            if show_steps is True:
                self.zoomable_mobs.remove(self.hypotenuses, self.altitudes,
                                          self.inner_lake)
                self.play(
            ...
            self.lake_center = self.obs_dot.get_center(
            ) + self.lake_radius * UP
            ...
            if show_steps is True:
                self.play(FadeIn(new_outer_lake, run_time=run_time),
                          FadeIn(self.ls0_dot))
            else:
                self.play(FadeIn(new_outer_lake, run_time=run_time), )
            ...
            self.zoomable_mobs.add(self.light_sources, self.inner_lake,
                                   self.outer_lake)
            ...
            if show_steps is True:
                ...
                self.zoomable_mobs.add(self.legs, self.hypotenuses,
                                       self.altitudes)
        ...
        self.zoomable_mobs.add(self.inner_lake, self.outer_lake,
                               self.altitudes, self.light_sources)
        ...
        for i in range(4):
            ...
            point = inner_lake_center + inner_lake_radius * np.array(
        ...
        for i in range(8):
            ...
            point = outer_lake_center + outer_lake_radius * np.array(
                [np.cos(theta), np.sin(theta), 0])
            dot = Dot(point, color=LAKE_STROKE_COLOR, radius=0.3)
            outer_ls.add(dot)
        ...
        inner_label = (Tex("\\theta",
                           fill_color=ANGLE_COLOR1).scale(3).next_to(
                               inner_angle_arc, LEFT, buff=-0.1))

        self.play(
            ShowCreation(line1),
            ShowCreation(line2),
        )
        self.play(ShowCreation(inner_angle_arc), FadeIn(inner_label))
        ...
        outer_label = (Tex(
            "{\\theta \over 2}",
            color=ANGLE_COLOR2).scale(2.5).move_to(outer_angle_arc))
        ...
        self.play(ShowCreation(outer_angle_arc), FadeIn(outer_label))
        ...
        self.play(FadeOut(inner_lines), FadeOut(inner_arcs))
        ...
        self.play(ShowCreation(outer_lines2), )

        self.wait()

        outer_lines3 = outer_lines2.copy().rotate(
            TAU / 2, about_point=outer_lake_center)
        outer_arcs3 = outer_arcs.copy().rotate(TAU / 2,
                                               about_point=outer_lake_center)

        self.play(ShowCreation(outer_lines3), )
...
class PondScene(ThreeDScene):
    ...
    def construct(self):
        ...
        # first lighthouse
        original_op_func = inverse_quadratic(LIGHT_MAX_INT, LIGHT_SCALE,
                                             LIGHT_CUTOFF)
        ls0 = LightSource(opacity_function=original_op_func,
                          radius=15.0,
                          num_levels=15)
        ...
        diameter_start = interpolate(OBSERVER_POINT, ls0.get_source_point(),
                                     0.02)
        ...
        def construction_step(n,
            ...
            if simultaneous_splitting is False:
                ...
                for i in range(2**n):
                    ...
                    if n == 1 and i == 0:
                        ...
                        right_angle1 = right_angle(A,
                                                   C,
                                                   B,
                                                   size=2 * RIGHT_ANGLE_SIZE)
                        ...
                        H = self.inner_lake.get_center(
                        ) + self.lake_radius / 2 * RIGHT
                        L = self.outer_lake.get_center()
                        triangle2 = triangle(L, H, C)

                        right_angle2 = right_angle(L,
                                                   H,
                                                   C,
                                                   size=2 * RIGHT_ANGLE_SIZE)
            ...
            # WE DON'T USE THIS CASE BRANCH ANYMORE
                ...
                self.play(
                    ReplacementTransform(old_ls,
                                         self.light_sources,
                                         run_time=run_time),
                    ReplacementTransform(old_lake,
                                         self.outer_lake,
                                         run_time=run_time),
                )
        ...
        self.play(FadeOut(angle_sign1), FadeOut(angle_sign2),
                  FadeOut(my_triangle))
        ...
        self.play(FadeOut(self.altitudes), FadeOut(self.hypotenuses),
                  FadeOut(self.legs))
        ...
        for n in range(3, max_it + 1):
            ...
            for i in range(2**n):
                ...
                if onscreen_old or onscreen_1:
                    ...
                    print("------------------ moving from", ls_old_loc[:2],
                          "to", pos1[:2])
                    shift_list.append(
                        ApplyMethod(ls1.move_source_to,
                                    pos1,
        ...
        self.number_line = NumberLine(
            ...
            origin_point -
            self.number_line.number_to_point(0))  # .shift(scale * 2.5 * DOWN)
        ...
        for (i, submob) in zip(list(range(nb_symbols)),
                               two_sided_sum.submobjects):
            submob.next_to(self.number_line.number_to_point(i - 13),
                           DOWN,
                           buff=2 * scale)
        ...
        randy = (Randolph(
            color=MAROON_D).scale(scale).move_to(2 * scale * DOWN +
                                                 5 * scale * LEFT))
...
class WaitScene(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.teacher_says(
            Tex("{1\over 1^2}+{1\over 3^2}+{1\over 5^2}+{1\over 7^2}+\dots = {\pi^2 \over 8}!"
                ))
...
class FinalSumManipulationScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(self.number_line1, run_time=5), )
        self.wait()
        ...
        self.play(ShowCreation(self.number_line2, run_time=5), )
        self.wait()
        ...
        self.play(
            LaggedStartMap(FadeIn, full_lighthouses, lag_ratio=0.2,
                           run_time=3), )

        self.play(
            LaggedStartMap(SwitchOn,
                           full_ambient_lights,
                           lag_ratio=0.2,
                           run_time=3))
        ...
        quarter_label = Tex("\\times {1\over 4}",
                            fill_color=LIGHT_COLOR2,
                            stroke_color=LIGHT_COLOR2)
        ...
        self.play(equation.shift,
                  -equation.get_center()[1] * UP + UP + 1.5 * LEFT)
...
class ArcHighlightOverlayScene(Scene):
    ...
    def construct(self):
        ...
        def flash_arcs(n):
            ...
            self.play(FadeOut(arcs[2**n], run_time=FLASH_TIME), )
...
class InfiniteCircleScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        C = B + 3.2 * UP
        right_line = DashedLine(B + 0.2 * DOWN,
                                C + 0.2 * UP,
                                stroke_color=YELLOW,
                                stroke_width=8)
...
class RightAnglesOverlay(Scene):
    ...
    def construct(self):
        ...
        for i in range(4):
            sign = right_angle(points[i - 1], lake_center, points[i],
                               RIGHT_ANGLE_SIZE)

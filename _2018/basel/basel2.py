    ...
    def build_up_euler_sum(self):
        ...
        for i, t1, t2 in zip(it.count(1), [0] + series_terms, series_terms):
            ...
            if i <= 5:
                ...
                rect_label_anim = ReplacementTransform(term_mobject.copy(),
                                                       rect_label)
            else:
...
class MathematicalWebOfConnections(PiCreatureScene):
    ...
    def complain_that_pi_is_not_about_circles(self):
        ...
        words = self.words = TexText("I am not", "fundamentally \\\\",
                                     "about circles")
    ...
    def show_other_pi_formulas(self):
        ...
        for formula in formulas[1:]:
            self.play(FadeIn(formula, lag_ratio=0.5, run_time=3), )
        self.wait()
    ...
    def show_web_of_connections(self):
        ...
        dots = VGroup(*[
            Dot(radius=0.1).move_to(
                (j - 0.5 * (i % 2)) * RIGHT + (np.sqrt(3) / 2.0) * i * DOWN +
                0.5 * (random.random() * RIGHT + random.random() * UP), )
            for i in range(4) for j in range(7 + (i % 2))
        ])
        dots.set_height(3)
...
class FirstLighthouseScene(PiCreatureScene):
    ...
    def show_lighthouses_on_number_line(self):
        ...
        self.play(morty.change, "pondering",
                  LaggedStartMap(FadeIn, lighthouses, run_time=1))
        self.play(
            LaggedStartMap(
                SwitchOn,
    ...
    def describe_brightness_of_each(self):
        ...
        triangle_anim = GrowFromPoint(triangle,
                                      triangle.get_right(),
                                      point_color=YELLOW)
        ...
        self.play(UpdateLightIndicator(light_indicator, 1),
        ...
        for i in range(1, 5):
            self.play(
                ...
                UpdateLightIndicator(light_indicator,
                                     1.0 / (i + 1)**2,
                                     run_time=3),
                FadeIn(
                    euler_sum_terms[i],
            ...
            self.wait()
        self.play(ApplyMethod(ls0.restore),
...
class IntroduceScreen(Scene):
    ...
    def rotate_screen(self):
        self.add(Mobject.add_updater(self.light_source,
                                     lambda m: m.update()), )
        self.add(
            Mobject.add_updater(
    ...
    def shoot_rays(self, show_creation_kwargs=None):
        ...
        rays = VGroup(*[
            Line(ORIGIN, self.radius * rotate_vector(RIGHT, angle)) for angle
            in np.arange(self.min_ray_angle, self.max_ray_angle, step_size)
        ])
        ...
        self.play(
            *[ApplyMethod(ray.set_color, ray.target_color) for ray in rays])
        self.wait()
...
class EarthScene(IntroduceScreen):
    ...
    def construct(self):
        ...
        screen.target.rotate(-60 * DEGREES,
                             about_point=earth_circle.get_center())
        ...
        pole_arrow.shift(screen.target.get_center() + SMALL_BUFF * LEFT -
                         pole_arrow.get_end())
...
class ShowLightInThreeDimensions(IntroduceScreen, ThreeDScene):
    ...
    def construct(self):
        ...
        cone = ThreeDSpotlight(screen, light_source.ambient_light,
...
class InverseSquareLaw(ThreeDScene):
    ...
    def move_screen_farther_away(self):
        ...
        screen.shift(source_point + unit_distance * RIGHT -
        ...
        light_indicator_update = Mobject.add_updater(light_indicator,
        ...
        self.set_variables_as_attrs(ambient_light, spotlight, shadow,
                                    lighthouse, morty, arrow_group,
                                    *continual_updates)
    ...
    def morph_into_3d(self):
        ...
        cone = ThreeDSpotlight(new_screen,
                               ambient_light,
        ...
        self.move_camera(
            ...
            added_anims=[
                ...
                #     run_time = 2,
                # ),
                ApplyFunction(lambda m: m.fade(1).shift(1.5 * DOWN),
                              light_indicator,
                              remover=True),
                FadeOut(morty),
            ],
        ...
        screen_copy_groups = list(map(get_screen_copy_group, list(range(1,
                                                                        8))))
        ...
        self.play(
            *[ApplyMethod(mob.fade, 1) for mob in screen_copy_groups[:2]])
...
class ScreensIntroWrapper(TeacherStudentsScene):
    ...
    def construct(self):
        point = VectorizedPoint(FRAME_X_RADIUS * LEFT / 2 +
                                FRAME_Y_RADIUS * UP / 2)
...
class ManipulateLightsourceSetups(PiCreatureScene):
    ...
    def construct(self):
        ...
        def update_light_indicator(light_indicator):
            distance = get_norm(light_source.get_source_point() -
                                observer_point)
        ...
        self.play(
            UpdateFromAlphaFunc(
                light_source,
                lambda ls, a: ls.move_to(observer_point + rotate_vector(
                    unit_distance * RIGHT, (1 + 1.0 / 8) * a * TAU)),
                run_time=6,
        ...
        self.play(
            ...
            ReplacementTransform(light_source.copy().fade(1),
            ...
            UpdateFromAlphaFunc(
                light_indicator,
                lambda m, a: m.move_to(point + a * 0.75 * RIGHT, ),
            ),
            UpdateFromAlphaFunc(
                light_indicator_copy,
                lambda m, a: m.move_to(point + a * 0.75 * LEFT, ),
            ),
            run_time=2,
        )
...
class TwoLightSourcesScene(ManipulateLightsourceSetups):
    ...
    def construct(self):
        ...
        morty.target.next_to(origin_point,
                             LEFT,
                             buff=0,
        ...
        # find the coords of the altitude point H
        # as the solution of a certain LSE
        prelim_matrix = np.array([[yA - yB, xB - xA], [xA - xB,
                                                       yA - yB]])  # sic
        ...
        def update_indicator(indicator):
            ...
            for ls in lsA, lsB, lsC:
                if ls in self.mobjects:
                    ...
                    d_indensity = fdiv(3.0 / (distance**2),
                                       indicator.opacity_for_unit_intensity)
                    d_indensity *= ls.ambient_light.submobjects[
                        1].get_fill_opacity()
                    intensity += d_indensity
            indicator.set_intensity(intensity)

        indicator_update_anim = Mobject.add_updater(indicator,
                                                    update_indicator)
        ...
        intensity = (lsA.ambient_light.opacity_function(distance1) /
                     indicator.opacity_for_unit_intensity)
        distance2 = get_norm(origin_point - lsB.get_source_point())
        intensity += (lsB.ambient_light.opacity_function(distance2) /
                      indicator.opacity_for_unit_intensity)

        # IPT Theorem
        theorem = Tex("{1 \over ", "a^2}", "+", "{1 \over", "b^2}", "=",
                      "{1 \over", "h^2}")
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(indicator,
                                lambda i, a: i.set_intensity(a * intensity)),
        )
...
class SimpleIPTProof(Scene):
    ...
    def construct(self):
        ...
        for arrow, line in zip(argument_lines[1::2], argument_lines[2::2]):
            line.save_state()
            line.shift(prev_line.alignment_mob.get_center() -
...
class WeCanHaveMoreFunThanThat(TeacherStudentsScene):
    ...
    def construct(self):
        point = VectorizedPoint(FRAME_X_RADIUS * LEFT / 2 +
                                FRAME_Y_RADIUS * UP / 2)
        self.teacher_says("We can have \\\\ more fun than that!",
                          target_mode="hooray")
...
class IPTScene(TwoLightSourcesScene, ZoomedScene):
    ...
    def construct(self):
        ...
        spotlights_update_anim = Mobject.add_updater(spotlights,
                                                     update_spotlights)
        ...
        self.play(*list(
            map(FadeOut, [spotlight_a, lsA.ambient_light, lsB.ambient_light])))
...
class DiameterTheorem(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        for angle in 0.2 * TAU, -0.4 * TAU, 0.3 * TAU:
            ...
            perp_mark.target.shift(point.target.get_center() -
...
class PondScene(ThreeDScene):
    ...
    def construct(self):
        ...
        ls0_dot = self.ls0_dot = Dot(OBSERVER_POINT + 2 * LAKE0_RADIUS * UP,
                                     fill_color=WHITE)
        ...
        self.play(morty.restore)
        self.play(GrowFromCenter(obs_dot), GrowFromCenter(ls0_dot),
        ...
        self.play(
            ...
            ReplacementTransform(new_diameter_text_copy,
                                 new_reading,
        ...
        randy = (Randolph(
            color=MAROON_E).scale(scale).move_to(2 * scale * DOWN +
                                                 5 * scale * LEFT))
...
class ThinkBackToHowAmazingThisIs(ThreeDScene):
    ...
    def show_sum(self):
        ...
        zoom_out = always_shift(self.camera.rotation_mobject,
                                direction=OUT,
                                rate=0.4)
        ...
        self.play(*list(
            map(
                FadeOut,
                it.chain(term_mobjects, plusses, number_line.numbers,
                         [decimal]),
            )))
    ...
    def show_giant_circle(self):
        ...
        circle.apply_function(lambda x_y_z: complex_to_R3(7 * np.exp(
            complex(0, 0.0315 * x_y_z[0]))))
...
class Credits(Scene):
    ...
    def construct(self):
        credits = VGroup(*[
            VGroup(*list(map(TexText, pair))) for pair in [

...
class ClosedLoopScene(Scene):
    ...
    def get_default_loop(self):
        loop = VMobject()
        loop.set_points_smoothly(self.loop_anchor_points +
                                 [self.loop_anchor_points[0]])
        return loop
    ...
    def add_dots_at_alphas(self, *alphas):
        self.add_dots(*[
            Dot(self.loop.point_from_proportion(alpha), color=self.dot_color)
            for alpha in alphas
        ])
    ...
    def add_connecting_lines(self, cyclic=False):
        ...
        for d1, d2 in pairs:
            ...
            line.update_anim = UpdateFromFunc(
                line,
                lambda l: l.put_start_and_end_on(l.start_dot.get_center(),
    ...
    def get_line_anims(self):
        return [line.update_anim
                for line in self.connecting_lines] + [Animation(self.dots)]
    ...
    def transform_loop(self, target_loop, added_anims=[], **kwargs):
        ...
        self.play(Transform(self.loop, target_loop),
                  *dot_anims + self.get_line_anims() + added_anims, **kwargs)

    def set_color_dots_by_pair(self):
        n_pairs = len(list(self.dots)) / 2
        for d1, d2, c in zip(self.dots[:n_pairs], self.dots[n_pairs:],
                             self.pair_colors):
    ...
    def find_square(self):
        ...
        quads = np.array(
            [[self.loop.point_from_proportion(alpha) for alpha in quad]
    ...
    def square_scores(self, all_quads):
        midpoint_diffs = np.apply_along_axis(
            get_norm,
            1,
            0.5 * (all_quads[:, 0] + all_quads[:, 2]) - 0.5 *
            (all_quads[:, 1] + all_quads[:, 3]),
        )
        ...
        dots = np.abs(unit_v1s[:, 0] * unit_v2s[:, 0] +
                      unit_v1s[:, 1] * unit_v2s[:, 1] +
                      unit_v1s[:, 2] * unit_v2s[:, 2])
...
class Introduction(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.random_blink(3)
        self.teacher_says("Here's why \\\\ I'm excited...",
                          target_mode="hooray")
        for pi in self.get_students():
...
class WhenIWasAKid(TeacherStudentsScene):
    ...
    def grow_up(self):
        ...
        self.play(
            ...
            *list(
                map(FadeOut, [
                    self.students, self.teacher, me.bubble, me.bubble.content
                ])))
        self.remove(me)
        self.add(morty)
...
class DefineInscribedSquareProblem(ClosedLoopScene):
    ...
    def cycle_through_shapes(self):
        ...
        for anim in randy_anims:
            self.let_dots_wonder(run_time=1.5,
                                 random_seed=0,
                                 added_anims=[anim])
        self.remove(randy)
    ...
    def ask_about_rectangles(self):
        ...
        dot_pairs = [
            VGroup(self.dots[i], self.dots[j]) for i, j in [(0, 2), (1, 3)]
        ]
        pair_colors = MAROON_B, PURPLE_B
        diag_lines = [
...
class RectangleProperties(Scene):
    ...
    def construct(self):
        ...
        dot_pairs = [
            VGroup(vertex_dots[i], vertex_dots[j]) for i, j in [(0, 2), (1, 3)]
        ]
    ...
    def construct(self):
        ...
        lines = [
            Line(dots[i].get_center(),
                 dots[j].get_center(),
                 color=dots[i].get_color()) for i, j in [(0, 1), (2, 3)]
        ]
        groups = [
        ...
        self.wait()
        self.play(*[
            ApplyMethod(group.shift, -group[-1].get_center() +
                        midpoint.get_center()) for group in groups
        ])
...
class SearchForRectangleOnLoop(ClosedLoopScene):
    ...
    def construct(self):
        ...
        midpoint = Dot(center_of_mass([d.get_center() for d in self.dots]),
                       color=RED)
...
class DeclareFunction(ClosedLoopScene):
    ...
    def construct(self):
        ...
        self.set_color_dots_by_pair()
        self.add_connecting_lines()
        VGroup(self.loop, self.dots,
               self.connecting_lines).scale(0.7).to_edge(LEFT).shift(DOWN)
...
class PairOfRealsToPlane(Scene):
    ...
    def construct(self):
        ...
        lines = VGroup(*[
            DashedLine(dot, two_d_point, color=dot.get_color()) for dot in dots
        ])
        self.play(*list(map(ShowCreation, lines)))
...
class SeekSurfaceForPairs(ClosedLoopScene):
    ...
    def construct(self):
        ...
        for anim in anims:
            self.let_dots_wonder(random_seed=1,
                                 added_anims=[anim],
...
class DefineUnorderedPair(ClosedLoopScene):
    ...
    def construct(self):
        ...
        arrows = [
            Arrow(*pair, color=PURPLE_B) for pair in it.product(labels, dots)
        ]
...
class DeformToInterval(ClosedLoopScene):
    ...
    def construct(self):
        ...
        end_dots = VGroup(self.dots[0], self.dots[-1], dots_copy[0],
                          dots_copy[-1])
...
class RepresentPairInUnitSquare(ClosedLoopScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(square), Animation(self.dots),
        ...
        for shift_val in shift_vals:
            ...
            for line, dot in zip(dashed_lines, self.dots):
                line.target.put_start_and_end_on(dot.target.get_center(),
...
class EdgesOfSquare(Scene):
    ...
    def get_coordinate_labels(self, square):
        ...
        label_groups = [
            VGroup(*[Tex("(%s, %s)" % (a, b)).scale(0.7) for b in alpha_range])
            for a in (0, 1)
        ]
    ...
    def get_arrows(self, x_edges, y_edges):
        ...
        return [
            ...
            ]).set_color(edges.get_color()) for edges in (x_edges, y_edges)
        ]
...
class EndpointsGluedTogether(ClosedLoopScene):
    ...
    def construct(self):
        ...
        dots = VGroup(
            *
            [Dot(line.get_bounding_box_point(vect)) for vect in (LEFT, RIGHT)])
        dots.set_color(BLUE)
        ...
        self.transform_loop(
            original_loop,
            added_anims=[
                ApplyMethod(dot.move_to,
                            original_loop.get_points()[0]) for dot in dots
            ],
            run_time=3,
        )
        self.wait()
...
class TrivialPairCollision(ClosedLoopScene):
    ...
    def construct(self):
        ...
        line = Line(a.get_corner(DOWN + LEFT),
...
class FoldUnitSquare(EdgesOfSquare):
    ...
    def add_triangles(self):
        ...
        triangles.set_fill(color=square.get_color(),
    ...
    def add_arrows(self):
        ...
        self.end_arrows = VGroup(*list(end_arrows[3:]) +
                                 list(end_arrows[:3])).copy()
    ...
    def ask_about_gluing(self):
        keepers = VGroup(self.triangles[0], self.start_arrows,
                         self.diag_line).copy()
        ...
        self.play(keepers.scale, 0.6, keepers.shift, 4 * RIGHT + UP,
                  FadeIn(randy))
        self.play(
            randy.change_mode,
            "pondering",
...
class PrepareForMobiusStrip(Scene):
    ...
    def rearrange_pieces(self):
        ...
        self.wait()
        self.play(*[
            ApplyMethod(tri.shift,
                        tri.get_points()[0][0] * LEFT)
            for tri in self.triangles
        ])
...
class PatreonThanks(Scene):
    ...
    def construct(self):
        ...
        left_patrons = VGroup(
            *list(map(TexText, self.specific_patrons[:n_patrons / 2])))
...
class CreditTWo(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(morty.look, LEFT, brother.change_mode, "happy", brother.look,
                  LEFT)
        self.play(Blink(morty))
        self.wait()
...
class CreditThree(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Blink(randy))
        self.play(randy.change_mode, "hooray", randy.look_at, logo_dot,
                  FadeOut(domains))
        self.wait()
        self.play(Write(whois), randy.change_mode, "confused", randy.look_at,
                  whois)
        self.wait(2)
